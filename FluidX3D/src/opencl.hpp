...
private:
	...
	inline void check_for_errors(const int error) {
		...
		if(error==-54) print_error("Workgrop size "+to_string((uint)cl_range_local.get()[0])+" for OpenCL kernel \""+name+"(...)\" is invalid! Maximum supported workgroup size on "+device->info.name+" is "+to_string(device->info.max_workgroup_size)+".");
