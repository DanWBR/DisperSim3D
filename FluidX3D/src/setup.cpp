...
// DLL build: main_setup() is unused (entry comes from disp_bridge.cpp via DllMain
// machinery, not the LBM bootstrap), but setup.hpp declares it so the linker
// needs a definition. Provide an empty one when none of the bundled benchmark
// flags are active.
