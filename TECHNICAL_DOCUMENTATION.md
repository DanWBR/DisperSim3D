...
6. **`Decoration3D.Model3D` is typed `object`** in engine, not `Model3DGroup`. Models hold the runtime WPF visual through an `object` slot; the UI layer casts on read (`deco.Model3D as Model3DGroup`). Saving/loading is unaffected because the visual is loaded from the `.obj`/`.stl`/`.rvm` path at runtime — it never serializes.
